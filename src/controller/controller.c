...
static int controller_method_list_unit_files_encode_reply(ListUnitFilesRequest *req, sd_bus_message *reply) {
        int r = sd_bus_message_open_container(reply, SD_BUS_TYPE_ARRAY, "(" "s" "ss" ")");
        if (r < 0) {
                return r;
        }
        ...
        for (int i = 0; i < req->n_sub_req; i++) {
                ...
                while (sd_bus_message_at_end(m, false) == 0) {
                        ...
                        r = sd_bus_message_enter_container(m, SD_BUS_TYPE_STRUCT, "ss");
                        if (r < 0) {
                                return r;
                        }
...
static const sd_bus_vtable controller_vtable[] = {
        ...
        SD_BUS_METHOD("ListUnitFiles", "", "a(sss)", controller_method_list_unit_files, 0),
